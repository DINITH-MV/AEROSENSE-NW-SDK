...
MDNS_SERVICE_TYPE = "_aerosense._tcp.local."
MDNS_SERVICE_NAME = "AeroSense Drone"

# Cache configuration
CACHE_DIR = Path.home() / ".aerosense"
...
class AeroSenseServiceListener:
    ...
    def add_service(self, zc, type_, name):
        ...
        if info:
            ...
            if info.addresses:
                ...
                if not drone_name:
                    # Extract from service name (e.g., "cacatua._aerosense._tcp.local.")
...
def discover_via_mdns(timeout=5.0, verbose=True):
    ...
    try:
        zeroconf = Zeroconf()
        listener = AeroSenseServiceListener(verbose=verbose)
        
        # Browse for AeroSense services
...
def probe_single_ip(ip, timeout=1.5):
    """
    Probe a single IP for AeroSense drone.
...
def scan_subnet_for_drones_parallel(local_ips, timeout=1.5, verbose=True, max_workers=50):
    """
    Scan subnet for AeroSense drones using parallel UDP probing.
    ...
    if verbose:
        print("Scanning subnet for AeroSense drones (parallel mode)...")
...
def discover_all_drones(timeout=5.0, verbose=True, use_cache=True, max_retries=2):
    """
    Discover all AeroSense drones on the network with enhanced robustness.
    ...
    if verbose:
        if found_drones:
            ...
            print("  1. Ensure drone AeroSense app is running")
