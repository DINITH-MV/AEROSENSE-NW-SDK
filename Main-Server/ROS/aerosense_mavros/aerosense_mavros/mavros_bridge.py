...
try:
    ...
    from aerosense_mavros.dji_interface import DJIInterface


class AeroSenseMavrosNode(Node):
    """
    MAVROS-compatible ROS 2 node for AeroSense DJI drone control.
    
    Publishes MAVROS-style topics and provides MAVROS-compatible services
    while using AeroSense's TCP/HTTP interface under the hood.
    """
    ...
    def __init__(self):
        super().__init__('aerosense_mavros')
        
        self.get_logger().info("Initializing AeroSense MAVROS Bridge...")
        ...
        self.get_logger().info(f"AeroSense MAVROS Bridge initialized - Connected to {self.drone_ip}")
    ...
    def _create_services(self):
        ...
        # AeroSense-specific services
        self.enable_vs_srv = self.create_service(
            Trigger, 'aerosense/enable_virtual_stick', self._enable_virtual_stick_callback)
        self.abort_mission_srv = self.create_service(
            Trigger, 'aerosense/abort_mission', self._abort_mission_callback)
    ...
    def _enable_virtual_stick_callback(self, request: Trigger.Request, response: Trigger.Response):
        """AeroSense-specific: Enable virtual stick mode."""
    ...
    def _abort_mission_callback(self, request: Trigger.Request, response: Trigger.Response):
        """AeroSense-specific: Abort current mission."""
    ...
    def destroy_node(self):
        """Clean shutdown."""
        self.get_logger().info("Shutting down AeroSense MAVROS Bridge...")
