# AeroSense MAVROS Bridge

A MAVROS-compatible ROS 2 interface for controlling DJI drones through AeroSense.
...
```bash
cd ~/ros2_ws/src
# Assuming AeroSense is already cloned
cd AeroSense/Main-Server/ROS

# Build the package
cd ~/ros2_ws
colcon build --packages-select aerosense_mavros
...
```bash
ros2 launch aerosense_mavros mavros_bridge.launch.py drone_ip:=192.168.1.100
```
...
```bash
ros2 launch aerosense_mavros multi_drone.launch.py
```
...
### AeroSense-specific Topics

| Topic                             | Description                     |
| --------------------------------- | ------------------------------- |
| `aerosense/waypoint_reached`      | Waypoint reached flag           |
| `aerosense/distance_to_home`      | Distance to home (meters)       |
| `aerosense/flight_time_remaining` | Estimated remaining flight time |
...
| `aerosense/enable_virtual_stick` | Trigger | Enable virtual stick                  |
| `aerosense/abort_mission`        | Trigger | Abort current mission                 |
...
| ---------------- | ------------- | ------------------------------ |
| `drone_ip`       | 192.168.1.100 | AeroSense RC IP address        |
...
MIT License - Part of the AeroSense project.
