...
//                add(Manifest.permission.READ_MEDIA_AUDIO)
    ...
    private fun updateAeroSenseBuildInfo() {
        val preferences = getSharedPreferences(AEROSENSE_PREFS_NAME, MODE_PRIVATE)
        val droneName = preferences.getString(AEROSENSE_PREF_DRONE_NAME, AEROSENSE_DEFAULT_DRONE_NAME)
            ?.takeIf { it.isNotBlank() }
            ?: AEROSENSE_DEFAULT_DRONE_NAME
        binding.textViewAerosenseBuildInfo.text = buildString {
            append("AeroSense app")
            append('\n').append("Drone: ").append(droneName)
            append('\n').append("Built: ").append(BuildConfig.AEROSENSE_BUILD_TIME)
            append('\n').append("Git: ").append(BuildConfig.AEROSENSE_GIT_SHA).append(' ').append(BuildConfig.AEROSENSE_GIT_STATE)
            append('\n').append("Version: ").append(BuildConfig.VERSION_NAME).append(" (").append(BuildConfig.VERSION_CODE).append(')')
            append('\n').append("Features: ").append(BuildConfig.AEROSENSE_FEATURES)
        }
    }
...
private const val AEROSENSE_PREFS_NAME = "AeroSensePrefs"
private const val AEROSENSE_PREF_DRONE_NAME = "drone_name"
private const val AEROSENSE_DEFAULT_DRONE_NAME = "drone_1"