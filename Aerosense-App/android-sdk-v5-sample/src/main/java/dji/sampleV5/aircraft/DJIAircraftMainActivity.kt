...
class DJIAircraftMainActivity : DJIMainActivity() {
    ...
    override fun prepareUxActivity() {
        ...
        enableDefaultLayout(AeroSenseDefaultLayoutActivity::class.java)
    ...
    private fun showNoDroneRecoveryPrompt() {
        ...
        noDronePrompt = AlertDialog.Builder(this)
            ...
            .setMessage("AeroSense still does not see a drone. Make sure the drone is on, the controller is on, the controller USB-C port is connected properly, and DJI Fly or any other DJI app is not running in the background, then reopen AeroSense to retry detection.")
            .setPositiveButton("Reopen AeroSense") { _, _ ->
                reopenAeroSense()
            }
