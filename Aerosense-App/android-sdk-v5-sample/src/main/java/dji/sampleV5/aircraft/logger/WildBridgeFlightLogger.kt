...
/**
 * AeroSenseFlightLogger — persists flight event data to a location that survives
 * app uninstalls and device reflashing.
 *
 * === AeroSense JSONL logs ===
 * Written to the best available storage (priority order):
 *   1. Removable microSD card root → AeroSense/FlightLogs/YYYY-MM-DD/ (needs MANAGE_EXTERNAL_STORAGE on API 30+)
 *   2. Documents/AeroSense/FlightLogs/YYYY-MM-DD/  (needs MANAGE_EXTERNAL_STORAGE on API 30+)
 ...
 *   AeroSense/DJI_FlightRecords/  (same storage-priority chain as above)
 * Already-copied files are skipped by filename, so the sync is always safe to repeat.
 */
object AeroSenseFlightLogger {

    private const val TAG = "AeroSenseFlightLogger"
    ...
    /**
     * Sync DJI SDK-managed flight records from [djiLogPath] to the AeroSense log root.
     *
     * This is a simple one-way mirror: any TXT/CSV/CLOG file in the DJI folder that does
     * not already exist (by name) in the AeroSense destination is copied over.
     ...
     * `AeroSense/DJI_FlightRecords/` — a flat, date-independent folder so all
     * DJI logs live in one place regardless of when they were produced.
     */
    private fun resolveDjiSyncDir(): File? {
        val subPath = "AeroSense/DJI_FlightRecords"
    ...
    private fun resolveLogDir(): File? {
        ...
        val subPath = "AeroSense/FlightLogs/$dateStr"
        val ctx = ContextUtil.getContext()
        val hasFullAccess = Build.VERSION.SDK_INT < Build.VERSION_CODES.R ||
                Environment.isExternalStorageManager()
        ...
        // 2. Public Documents/AeroSense/FlightLogs — needs MANAGE_EXTERNAL_STORAGE on API 30+.
        if (hasFullAccess) {
            val documentsRoot = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS)
            val dir = File(documentsRoot, "AeroSense/FlightLogs/$dateStr")
            if (dir.mkdirs() || dir.isDirectory) {
