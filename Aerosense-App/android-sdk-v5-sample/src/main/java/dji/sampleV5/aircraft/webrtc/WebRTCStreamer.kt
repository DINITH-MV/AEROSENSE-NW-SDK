...
class WebRTCStreamer(
    ...
    /** The currently active mock capturer, non-null only when mock video is in use. */
    @Volatile var activeMockCapturer: MockMp4VideoCapturer? = null
        private set
    /** Forwarded to each MockMp4VideoCapturer so the in-app preview mirrors the WebRTC frames. */
    var mockBitmapFrameListener: ((android.graphics.Bitmap) -> Unit)? = null
        set(value) {
            field = value
            activeMockCapturer?.bitmapFrameListener = value
        }
    ...
    private fun createVideoCapturer(clientId: String): VideoCapturer {
        return if (useMockVideo) {
            MockMp4VideoCapturer(droneName).apply {
                ...
                bitmapFrameListener = mockBitmapFrameListener
            }.also { activeMockCapturer = it }
        } else {
            activeMockCapturer = null
