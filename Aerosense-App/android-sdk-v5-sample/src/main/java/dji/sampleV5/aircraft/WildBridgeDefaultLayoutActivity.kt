...
import dji.sampleV5.aircraft.logger.AeroSenseFlightLogger
...
/**
 * AeroSense Default Layout Activity
 * 
 ...
 */
class AeroSenseDefaultLayoutActivity : DefaultLayoutActivity() {

    companion object {
        private const val TAG = "AeroSenseDefaultLayout"
        ...
        private const val DISCOVERY_MSG = "DISCOVER_AEROSENSE"
        private const val DISCOVERY_RESPONSE_PREFIX = "AEROSENSE_HERE:"
        private const val MULTICAST_GROUP = "239.255.42.99"
        private const val MULTICAST_PORT = 30001
        
        // mDNS/Zeroconf service type
        private const val MDNS_SERVICE_TYPE = "_aerosense._tcp."
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sharedPreferences = getSharedPreferences("AeroSensePrefs", Context.MODE_PRIVATE)
        ...
        multicastLock = wifiManager?.createMulticastLock("AeroSenseMulticastLock")
    ...
    private fun startMockVideoPreview(surfaceTexture: android.graphics.SurfaceTexture) {
        if (!(shouldAllowMockVideo() && isMockVideoEnabled())) return
        // Wire the WebRTC capturer's bitmap frames directly to the preview TextureView.
        // Canvas drawing is posted to the main thread so it never blocks the PTS timing loop.
        val preview = findViewById<android.view.TextureView>(R.id.mock_video_preview) ?: return
        val latestBitmap = java.util.concurrent.atomic.AtomicReference<android.graphics.Bitmap?>(null)
        val drawPending = java.util.concurrent.atomic.AtomicBoolean(false)
        webRTCStreamer?.mockBitmapFrameListener = { bitmap ->
            latestBitmap.set(bitmap)
            if (drawPending.compareAndSet(false, true)) {
                mainHandler.post {
                    drawPending.set(false)
                    val bmp = latestBitmap.get() ?: return@post
                    val canvas = preview.lockCanvas()
                    if (canvas != null) {
                        try {
                            val src = android.graphics.Rect(0, 0, bmp.width, bmp.height)
                            val dst = android.graphics.RectF(0f, 0f, canvas.width.toFloat(), canvas.height.toFloat())
                            canvas.drawBitmap(bmp, src, dst, null)
                        } finally {
                            preview.unlockCanvasAndPost(canvas)
                        }
                    }
                }
            }
    ...
    private fun stopMockVideoPreview() {
        webRTCStreamer?.mockBitmapFrameListener = null
    }
    ...
    private fun setupDroneStatusView() {
        DroneController.droneStatusListener = object : DroneController.DroneStatusListener {
            override fun onDroneStatusChanged(status: DroneController.DroneStatus) {
                AeroSenseFlightLogger.logStatus(status.name)
    ...
    /**
     * Copy DJI SDK-managed TXT flight records into the AeroSense DJI_FlightRecords folder.
    ...
    private fun syncDjiFlightLogsInBackground() {
        Thread {
            try {
                ...
                val count = AeroSenseFlightLogger.syncDjiFlightLogs(djiPath)
                if (count > 0) {
                    mainHandler.post {
                        Toast.makeText(this, "Synced $count DJI flight log(s) to AeroSense folder", Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
    ...
    private fun setupDroneNameDisplay() {
        ...
        findViewById<ImageButton>(R.id.button_aerosense_settings)?.setOnClickListener { anchor ->
            showAeroSenseSettingsMenu(anchor)
        }
    }

    private fun showAeroSenseSettingsMenu(anchor: android.view.View) {
        ...
        PopupMenu(this, anchor).apply {
            ...
            setOnMenuItemClickListener { item -> handleAeroSenseMenuItem(item.itemId) }
            show()
        }
    }
    ...
    private fun setupKeyListeners() {
        ...
        KeyManager.getInstance().listen(isFlyingKey, this) { _, newValue ->
            ...
            if (!wasFlying && flying) {
                AeroSenseFlightLogger.startSession()
                ...
                mainHandler.postDelayed({
                    if (!DroneController.isAirborne) {
                        AeroSenseFlightLogger.endSession("landed")
    ...
    private fun loadDroneName() {
        ...
        if (storedName.isEmpty()) {
            ...
            AeroSenseFlightLogger.setDroneName(droneName)
        }
    }
    ...
    private fun formatDroneStorage(location: CameraStorageLocation, label: String) {
        ...
        KeyManager.getInstance().performAction(key, location, object : CommonCallbacks.CompletionCallbackWithParam<EmptyMsg> {
            override fun onSuccess(result: EmptyMsg?) {
                mainHandler.post {
                    Toast.makeText(this@AeroSenseDefaultLayoutActivity, "$label formatted", Toast.LENGTH_LONG).show()
                }
            ...
            override fun onFailure(error: IDJIError) {
                ...
                mainHandler.post {
                    Toast.makeText(this@AeroSenseDefaultLayoutActivity, message, Toast.LENGTH_LONG).show()
                }
                Log.e(TAG, message)
            }
        })
    ...
    override fun onDestroy() {
        ...
        try {
            ...
            AeroSenseFlightLogger.endSession("app_stopped")
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return if (handleAeroSenseMenuItem(item.itemId)) true else super.onOptionsItemSelected(item)
    }

    private fun handleAeroSenseMenuItem(itemId: Int): Boolean {
        return when (itemId) {
            1 -> {
                showDroneNameDialog(isFirstTime = false)
                true
    ...
    private class MdnsRegistrationListener(activity: AeroSenseDefaultLayoutActivity) : NsdManager.RegistrationListener {
